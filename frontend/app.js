...
function buildSidebar(activePage) {
  ...
  // Inject logo into sidebar-logo if present
  ...
  const links = NAV.filter(l => l.always || (l.roles && l.roles.includes(user.role)));
  nav.innerHTML = links.map(l => `<a href="${l.href}" class="nav-link ${activePage===l.href?'active':''}"><span class="nav-icon">${l.icon}</span><span>${l.label}</span></a>`).join('');
