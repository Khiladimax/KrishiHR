...
/**
 * MovementFragment — Google Maps native implementation
 *
 * Map layers:
 *  1. Blue polyline   — road-snapped route via Google Directions API
 *  2. Orange circles  — interpolated dot every 500m along the road (E1→S2)
 *  3. Blue circles    — real GPS points (middle points)
 *  4. Green marker    — START (first GPS point)
 *  5. Red marker      — END (last GPS point, non-live)
 *  6. Walker BitmapDescriptor — current position (live mode)
 */
class MovementFragment : Fragment(), OnMapReadyCallback {
    ...
    private val liveRunnable = object : Runnable {
        override fun run() {
            if (isLiveMode && currentEmpId != -1) refreshLiveRoute()
            liveHandler.postDelayed(this, AndroidMain.MOVEMENT_LIVE_REFRESH_MS)
        }
    }

    // ── Coroutine scope for Directions API calls ──────────────────────────────
    ...
    override fun onResume() {
        super.onResume()
        if (isLiveMode && currentEmpId != -1) liveHandler.post(liveRunnable)
    }
    ...
    override fun onMapReady(map: GoogleMap) {
        ...
        // Render pending points if data arrived before map was ready
    ...
    private fun buildUI(): View {
        ...
        // Map container — Google Maps fragment goes here
        ...
        legendRow.addView(dot(Color.parseColor("#3b82f6"))); legendRow.addView(legendTxt("GPS Point"))
        legendRow.addView(dot(Color.parseColor("#f97316"))); legendRow.addView(legendTxt("500m mark"))
    ...
    private fun renderResults(emp: Employee, pts: List<MovementPoint>, animate: Boolean) {
        ...
        // KM (straight-line for stat card — road km comes from Directions response)
        var totalKm = 0.0
        ...
        val avgSpeedKmh: Double = if (pts.size >= 2) {
            try {
                ...
                val segKm = haversine(pts[pts.size-2].lat, pts[pts.size-2].lng,
                    pts.last().lat, pts.last().lng)
        ...
        listOf(
            ...
            S(pts.last().timeLabel,             if (isLiveMode) "Last Ping" else "End",
                R.color.accent_red,  R.color.accent_red_light),
        ...
        pts.forEachIndexed { i, pt ->
            ...
            if (!isLast) dotCol.addView(View(ctx).apply {
                layoutParams = llp(px(2,dp), px(24,dp)).also {
                    it.topMargin = px(2,dp); it.gravity = Gravity.CENTER_HORIZONTAL
                }
                setBackgroundColor(ContextCompat.getColor(ctx, R.color.divider))
            })
            row.addView(dotCol)
            val info = vLL(ctx).apply {
                layoutParams = llp(0, WRAP, 1f); setPadding(px(8,dp), 0, 0, 0)
            }
            val badge = when {
                isFirst -> "🟢 START"
    ...
    /**
     * Main map render. Called on main thread.
     * 1. Clears previous overlays
     * 2. Places START / END / GPS markers
     * 3. Fetches Google Directions road route for each segment (IO)
     * 4. Draws blue polyline + orange 500m dots on main thread
     */
    private fun renderOnMap(pts: List<MovementPoint>, animate: Boolean) {
        ...
        if (!isLiveMode && pts.size > 1) {
            markers += map.addMarker(MarkerOptions()
                ...
                .title("🔴 END")
                .snippet("🕐 ${pts.last().timeLabel}"))!!
        }

        // ── Walker marker (live) — cyan ───────────────────────────────────────
        if (isLiveMode) {
            markers += map.addMarker(MarkerOptions()
                .position(latLngs.last())
        ...
        // ── Fit camera to all GPS points ──────────────────────────────────────
        val bounds = LatLngBounds.Builder().apply { latLngs.forEach { include(it) } }.build()
        map.animateCamera(CameraUpdateFactory.newLatLngBounds(bounds, 80))

        // ── Fetch Directions road routes (all segments parallel) ──────────────
        if (pts.size < 2) return

        mapScope.launch {
            val segmentJobs = (1 until pts.size).map { idx ->
                async {
                    fetchDirectionsSegment(pts[idx - 1], pts[idx])
                }
            }
            val segments = segmentJobs.awaitAll()   // wait for all segments
            ...
            withContext(Dispatchers.Main) {
                ...
                segments.forEachIndexed { segIdx, roadCoords ->
                    ...
                    // ── Orange 500m interpolated dots (E1 → S2) ───────────────
                    val fromTime = pts[segIdx].timeLabel
                    val toTime   = pts[segIdx + 1].timeLabel
                    val dots     = interpolate500m(roadCoords)
                    dots.forEach { coord ->
                        orangeDots += map.addCircle(CircleOptions()
                            ...
                            .strokeColor(Color.WHITE)
                            .strokeWidth(2f)
                            .fillColor(Color.parseColor("#f97316")))   // orange-500
                        // Tap info via info window needs a marker — add a transparent one
                        markers += map.addMarker(MarkerOptions()
                            .position(coord)
                            .alpha(0f)          // invisible — just for info window
                            .title("🟠 500m mark")
    ...
    /**
     * Fetch road polyline for ONE segment via Google Directions API.
     * Returns list of LatLng decoded from the overview_polyline.
    ...
    private suspend fun fetchDirectionsSegment(
        ...
        try {
            ...
            // Fallback: straight line so map never breaks
            listOf(LatLng(from.lat, from.lng), LatLng(to.lat, to.lng))
        }
    }

    /**
     * Decode Google encoded polyline string → List<LatLng>
    ...
    /**
     * Walk a road polyline and place one point every 500m.
     * These are the orange E1→S2 dots.
     */
