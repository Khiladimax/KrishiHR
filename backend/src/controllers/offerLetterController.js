...
.cand-name  { font-size: 12pt; font-weight: bold; line-height: 1.6; }
.cand-line  { font-size: 12pt; line-height: 1.65; }

/* Salutation */
.salut { font-size: 12pt; margin: 16px 0 14px; }
...
.sal-tbl th.c-num { text-align: right; }
.sal-tbl td {
  border: 1px solid #555;
  padding: 6px 12px;
  font-family: 'Times New Roman', Times, serif;
}
.sal-tbl tbody tr:nth-child(even) { background: #f9f9f9; }
.c-sr  { text-align: center; width: 46px; }
.c-num { text-align: right; width: 96px; }

/* Key rows */
.r-gross td { font-weight: bold; background: #f0f0f0 !important; border-top: 1.5px solid #000; border-bottom: 1.5px solid #000; }
.r-ded   td { font-weight: bold; background: #f0f0f0 !important; border-top: 1.5px solid #000; border-bottom: 1.5px solid #000; }
.r-net   td { font-weight: bold; background: #e8e8e8 !important; border-top: 1.5px solid #000; border-bottom: 1.5px solid #000; }
.r-ctc   td { font-weight: bold; background: #d8d8d8 !important; border-top: 2px solid #000; border-bottom: 2px solid #000; }

/* Acknowledgement */
.ack-section { margin-top: 22px; page-break-inside: avoid; }
.ack-title {
  font-size: 13pt;
  font-weight: bold;
  text-decoration: underline;
...
.ack-gap { padding-left: 32px; }

/* ── SPACER between pages on screen ── */
@media screen {
...
@media print {
  ...
  /* Fixed header and footer using CSS paged media */
  .page-header {
  ...
  @page {
    size: A4;
    margin: 30mm 18mm 24mm 18mm;
    @top-center { content: element(header); }
    @bottom-center { content: element(footer); }
  }

  /* Fallback for wkhtmltopdf: repeat header/footer per .page block */
  .page-break { page-break-before: always; }
  .sec-hd { page-break-after: avoid; }
  .sig-table { page-break-inside: avoid; }
  .ack-section { page-break-inside: avoid; }
  .sal-tbl { page-break-inside: avoid; }
  ...
  // Shared header and footer blocks — appear on EVERY page div
  const header = `
<div class="page-header">
  <table class="hdr-table" cellpadding="0" cellspacing="0">
    <tr>
      <td class="hdr-logo-td" rowspan="3">
        <img src="${LOGO}" class="hdr-logo" alt="KCMS Logo">
      </td>
      <td class="hdr-company">Krishi Care &amp; Management Services Private Limited</td>
    </tr>
    <tr>
      <td class="hdr-address">
    ...
    <tr>
      <td class="hdr-address">
        Email: hr@krishicare.in &nbsp;&nbsp; Website: www.krishicare.in &nbsp;&nbsp; Tel. - +912268284109
      </td>
    </tr>
  </table>
  <hr class="hdr-rule-thick">
</div>`;

  const footer = `
<div class="page-footer">
  <hr class="ftr-rule">
  <p class="ftr-corp"><strong>Corporate Office:</strong> H-12, Green Park Extension, New Delhi - 110016. Tel: 011-41039506.</p>
    ...
    <table class="sig-table" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td class="sig-td">
          ...
          <div class="sig-name">Human Resource</div>
  ...
  <div class="page-body">
    ...
    <table class="sal-tbl">
      ...
      <tbody>
        ...
        ${conv>0?`<tr><td class="c-sr">3</td><td>Conveyance Allowances (Fixed)</td><td class="c-num">${fmtV(conv)}</td><td class="c-num">${fmtV(conv*12)}</td></tr>`:''}
        <tr><td class="c-sr">${conv>0?4:3}</td><td>Other Allowances (Balance amount)</td><td class="c-num">${fmtV(other)}</td><td class="c-num">${fmtV(other*12)}</td></tr>
        <tr><td class="c-sr">${conv>0?5:4}</td><td>Gratuity</td><td class="c-num">${fmtV(gratuity)}</td><td class="c-num">${fmtV(gratuity*12)}</td></tr>
        <tr class="r-gross"><td class="c-sr">5</td><td>Total Gross Salary</td><td class="c-num">${fmtV(gross)}</td><td class="c-num">${fmtV(gross*12)}</td></tr>
        ...
        <tr class="r-ded"><td class="c-sr">8</td><td>Total Deductions (PF+PT)</td><td class="c-num">${totalDed>0?fmtV(totalDed):'&ndash;'}</td><td class="c-num">${totalDed>0?fmtV(totalDed*12):'&ndash;'}</td></tr>
        <tr class="r-net"><td class="c-sr">9</td><td>Net Salary (Gross &minus; Total Deductions)</td><td class="c-num">${fmtV(netSalary)}</td><td class="c-num">${fmtV(netSalary*12)}</td></tr>
    ...
    <div class="ack-section">
      ...
      <table class="ack-table" cellpadding="0" cellspacing="0">
        <tr>
          <td class="ack-lbl">Signature:</td>
          <td class="ack-line-td">&nbsp;</td>
          <td class="ack-lbl ack-gap">Name:</td>
          <td class="ack-line-td">&nbsp;</td>
        </tr>
        <tr>
          <td class="ack-lbl" style="padding-top:22px;">Location:</td>
          <td class="ack-line-td" style="padding-top:22px;">&nbsp;</td>
          <td class="ack-lbl ack-gap" style="padding-top:22px;">Date:</td>
          <td class="ack-line-td" style="padding-top:22px;">&nbsp;</td>
        </tr>
      </table>
    </div>
