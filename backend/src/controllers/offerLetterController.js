...
/* BODY */
.body { padding: 14px 24px 10px; flex: 1; }
...
.cand-name { font-weight: bold; font-size: 12pt; }
.cand-line { font-size: 12pt; }

/* Salutation */
.salut { font-size: 12pt; margin: 12px 0 10px; }
...
/* Signatures */
.sig-into { font-size: 12pt; margin: 14px 0 8px; line-height: 1.7; }
.sig-row {
  display: flex;
...
/* Excel-style salary table — matches original screenshot exactly */
...
/* Row 1: "Gross salary per month" — yellow header */
.r-gross-hd td {
  background: #ffff00 !important;
  font-weight: bold;
  font-size: 11.5pt;
}
/* Row 2: "Components in salary" header row — white bold */
.r-col-hd td { font-weight: bold; }
/* Regular salary rows */
.r-normal td { background: #fff; }
/* "Total Gross Salary" — bold */
.r-total-gross td { font-weight: bold; }
/* PF/ESI/PT deduction rows — salmon/orange */
.r-ded-row td { background: #ffc7ce !important; color: #9c0006; }
/* "Total deductions" — orange bold */
...
/* Blank separator */
.r-blank td { background: #fff; border-left: none; border-right: none; border-top: none; }
/* "CTC Calculation" label */
.r-ctc-hd td { font-weight: bold; border: none; padding-top: 8px; }
/* CTC rows */
.r-ctc-row td { background: #fff; }
/* Final CTC row — yellow */
...
.ack-row   { display: flex; justify-content: space-between; margin-bottom: 18px; font-size: 12pt; }
.ack-field { width: 46%; }
.ack-label { font-size: 12pt; display: inline; }
...
@media print {
  ...
  // Shared header — exact match to original screenshot
  const hdr = `
<div class="hdr">
...
<div class="ftr">
  ...
  <p class="ftr-text"><strong>Corporate Office:</strong> H-12, Green Park Extension, New Delhi -110016. Tel: 011-41039506.</p>
  <p class="ftr-cin">CIN: ${COMPANY.cin}</p>
</div>`;
...
<div class="page">
  ...
  <div class="body">
    ...
    <div class="cand">
      ${ol.candidate_name ? `<p class="cand-name">${ol.candidate_name}</p>` : ''}
...
<div class="page page-break">
  ...
  <div class="body">
    ...
    <p class="sig-into">
      Yours truly,<br>
      From <strong>Krishi Care &amp; Management Services Private Limited,</strong>
    </p>
...
<div class="page page-break">
  ...
  <div class="body">
    ...
    <table class="sal">
      <tr class="r-gross-hd">
        <td colspan="2"><strong>Gross salary per month</strong></td>
        <td class="c-r" colspan="2"><strong>${fmtV(gross)}</strong></td>
      </tr>
      <tr class="r-col-hd">
        <td colspan="2"><strong>Components In salary</strong></td>
        <td class="c-r"><strong>Per month</strong></td>
        <td class="c-r"><strong>Per annum</strong></td>
      </tr>
      <tr class="r-normal"><td colspan="2">Basic Salary</td><td class="c-r">${fmtV(basic)}</td><td class="c-r">${fmtV(basic*12)}</td></tr>
      <tr class="r-normal"><td colspan="2">HRA (calculated on basic wage)</td><td class="c-r">${fmtV(hra)}</td><td class="c-r">${fmtV(hra*12)}</td></tr>
      <tr class="r-normal"><td colspan="2">Conveyance allowances (Fixed)</td><td class="c-r">${conv>0?fmtV(conv):'-'}</td><td class="c-r">${conv>0?fmtV(conv*12):'0'}</td></tr>
      <tr class="r-normal"><td colspan="2">Other allowances (Balance amount)</td><td class="c-r">${fmtV(other)}</td><td class="c-r">${fmtV(other*12)}</td></tr>
      <tr class="r-normal"><td colspan="2">Gratuity</td><td class="c-r">${fmtV(gratuity)}</td><td class="c-r">${fmtV(gratuity*12)}</td></tr>
      ...
      <tr class="r-ctc-row"><td colspan="2">Employer PF contribution</td><td class="c-r">${pfEmpr>0?fmtV(pfEmpr):'-'}</td><td class="c-r">${pfEmpr>0?fmtV(pfEmpr*12):'0'}</td></tr>
      <tr class="r-ctc-row"><td colspan="2">Employer PF contribution Admin charges</td><td class="c-r">${pfAdmin>0?fmtV(pfAdmin):'-'}</td><td class="c-r">${pfAdmin>0?fmtV(pfAdmin*12):'0'}</td></tr>
      <tr class="r-ctc-row"><td colspan="2">Employer ESI contribution</td><td class="c-r">-</td><td class="c-r">0</td></tr>
