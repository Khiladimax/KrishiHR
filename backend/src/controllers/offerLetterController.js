...
function buildOfferLetterHTML(ol) {
  ...
  // ── Header: shown in browser preview, hidden when Puppeteer prints ─────────
  // Puppeteer's headerTemplate handles the repeating letterhead on every PDF page.
  // In browser preview, this shows once at the top.
  const hdrHTML = `
  ...
  // ── Single continuous document — no manual page breaks in body ────────────
  // Puppeteer paginates naturally; only Annexure gets page-break-before.
  const mainLetter = `
<div class="doc-wrap">
  ${hdrHTML}
  <div class="doc-body">
    ...
    <!-- Annexure — always starts on new page -->
    <div class="annexure-section">
      ...
      <div class="ack-box">
        ...
        <table class="ack-tbl" cellpadding="0" cellspacing="0">
          <tr>
            <td class="ack-lbl">Signature:</td><td class="ack-line"></td>
            <td class="ack-lbl ack-gap">Date:</td><td class="ack-line"></td>
          </tr>
          <tr>
            <td class="ack-lbl" style="padding-top:20px;">Name:</td><td class="ack-line" style="padding-top:20px;"></td>
            <td class="ack-lbl ack-gap" style="padding-top:20px;">Location:</td><td class="ack-line" style="padding-top:20px;"></td>
          </tr>
        </table>
      </div>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo-img  { width: 75px; height: 75px; display: block; }
  .hdr-name  {
  ...
  /* In-body header: visible in browser, hidden when Puppeteer prints */
  .doc-header-preview { display: block; }
  ...
  @media print {
    ...
    /* Hide in-body header — Puppeteer's headerTemplate repeats it on every page */
    .doc-header-preview { display: none; }
    .annexure-section   { page-break-before: always; }
...
exports.sendEmail = async (req, res) => {
  try {
    ...
    // ── Build the offer letter HTML (browser preview version) ───────────────
    const offerHTML = buildOfferLetterHTML(ol);

    // ── Generate PDF via Puppeteer (modern Chrome engine) ───────────────────
    // Header/footer repeat automatically on every page — no manual page breaks,
    // no duplicate headers in body, no black gaps.
    let offerPdfBuffer = null;
    try {
      const puppeteer = require('puppeteer');

      // Logo as base64 for header template (no file path dependency)
      ...
      try {
        ...
        console.warn('[offerLetter] Logo file not found, header will have no logo');
      }

      // ── Header template — repeats on every PDF page automatically ──────────
      const headerTemplate = `
        <div style="width:100%;font-family:Arial,Helvetica,sans-serif;font-size:9px;color:#111;padding:6px 25mm 0;box-sizing:border-box;">
          <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
            <tr>
              <td width="72" rowspan="3" style="padding-right:10px;vertical-align:middle;">
                ${logoDataUri ? `<img src="${logoDataUri}" style="width:65px;height:65px;display:block;">` : ''}
              </td>
              <td style="font-size:14px;font-weight:900;color:#1a6b1a;vertical-align:bottom;padding-bottom:2px;">
                Krishi Care &amp; Management Services Private Limited
              </td>
            </tr>
            <tr>
              <td style="font-size:7.5px;text-align:center;color:#111;line-height:1.5;padding-top:1px;">
                <strong>Regd. &amp; Head Office:</strong> 617, 6th Floor, Hubtown Viva, Western Express Highway, Shankarwadi, Jogeshwari (East), Mumbai - 400060.
              </td>
            </tr>
            <tr>
              <td style="font-size:7px;text-align:center;color:#111;padding-top:1px;">
                Email: administrator@krishicare.in, Website: http://www.krishicare.com, Tel. - +91 22 68284109
              </td>
            </tr>
          </table>
          <div style="border-bottom:2px solid #1a6b1a;margin-top:5px;"></div>
        </div>`;

      // ── Footer template — repeats on every PDF page automatically ──────────
      const footerTemplate = `
        <div style="width:100%;font-family:Arial,Helvetica,sans-serif;font-size:7px;color:#111;padding:0 25mm;box-sizing:border-box;">
          <div style="border-top:1.5px solid #1a6b1a;margin-bottom:3px;"></div>
          <p style="text-align:center;margin:0;line-height:1.6;">
            <strong>Corporate Office:</strong> ${COMPANY.corpAddr}. Tel: 011-41039506.
          </p>
          <p style="text-align:center;margin:0;">
            <strong>CIN: ${COMPANY.cin}</strong>
          </p>
        </div>`;

      // ── Content HTML — no header/footer, just body content ─────────────────
      // We strip the letterhead from offerHTML since Puppeteer header handles it.
      // The offerHTML already has the content in .doc-body.
      ...
      await browser.close();
      console.log('[offerLetter.sendEmail] Puppeteer PDF generated, size:', offerPdfBuffer.length);
    } catch (pdfErr) {
      console.error('[offerLetter.sendEmail] Puppeteer PDF failed:', pdfErr.message);
    }
