...
function buildOfferLetterHTML(ol) {
  ...
  // ── HEADER HTML for wkhtmltopdf --header-html ────────────────────────────
...
.wrap { padding:8px 15mm 0 15mm; }
table { width:100%; border-collapse:collapse; }
.logo-td { width:90px; vertical-align:middle; padding-right:10px; }
.logo-img { width:80px; height:80px; object-fit:contain; display:block; }
.tag { font-size:7pt; font-weight:bold; color:#000; line-height:1.25; margin-top:3px; text-align:center; width:80px; }
.name-td { vertical-align:top; padding-top:4px; }
.company { font-size:18pt; font-weight:bold; color:#000; line-height:1.15; }
.addr { font-size:9pt; color:#000; line-height:1.5; margin-top:3px; }
.rule { border-top:1.5px solid #000; margin-top:6px; }
</style></head><body>
<div class="wrap">
<table cellpadding="0" cellspacing="0"><tr>
  <td class="logo-td">
    <img src="${LOGO}" class="logo-img" alt="">
    <div class="tag">BRINGING KRISHI &amp;<br>VIGYAN TOGETHER</div>
  </td>
  <td class="name-td">
    <div class="company">Krishi Care &amp; Management Services Private Limited</div>
    <div class="addr"><strong>Regd. &amp; Head Office:</strong> 617, 6th Floor, Hubtown Viva, Western Express Highway, Shankarwadi, Jogeshwari (East), Mumbai - 400060.</div>
    <div class="addr">Email: recruitments@krishicare.in, Website: www.krishicare.in, Tel. - +912268284109</div>
  </td>
</tr></table>
<div class="rule"></div>
</div>
  ...
  // ── FOOTER HTML for wkhtmltopdf --footer-html ────────────────────────────
  const footerHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
* { margin:0; padding:0; box-sizing:border-box; }
body { font-family:'Times New Roman',Times,serif; font-size:9pt; margin:0; padding:0; }
.wrap { padding:4px 15mm 6px 15mm; }
.rule { border-top:1px solid #000; margin-bottom:4px; }
.corp { text-align:center; font-size:9pt; }
.cin  { text-align:center; font-size:9pt; font-weight:bold; }
</style></head><body>
<div class="wrap">
  <div class="rule"></div>
  ...
  // ── BODY CSS (no header/footer — wkhtmltopdf injects them) ───────────────
  const css = `
...
.date-line { text-align:right; font-weight:bold; margin-bottom:16px; font-size:12pt; }
.cand  { margin-bottom:14px; line-height:1.65; font-size:12pt; }
.cand-name { font-weight:bold; }
.salut { margin:12px 0 10px; font-size:12pt; }
.subj  { font-weight:bold; text-align:center; text-decoration:underline; margin:10px 0 14px; font-size:12pt; }
...
.sig-intro { margin:16px 0 8px; line-height:1.7; font-size:12pt; }
...
.sig-lbl  { font-weight:bold; font-size:12pt; }
.sig-sub  { font-size:12pt; }
/* Annexure */
.ann-title { text-align:center; font-weight:bold; margin:14px 0 12px; font-size:12pt; }
.ann-meta  { font-weight:bold; line-height:1.9; margin-bottom:14px; font-size:12pt; }
.sal { width:80%; margin:0 auto 18px; border-collapse:collapse; font-size:11pt; }
.sal th { border:1px solid #000; padding:6px 10px; text-align:center; font-weight:bold; font-family:'Times New Roman',Times,serif; }
.sal td { border:1px solid #000; padding:5px 10px; font-family:'Times New Roman',Times,serif; }
.sal .c-sr  { text-align:center; width:46px; }
.sal .c-num { text-align:right; width:90px; }
.r-bold td  { font-weight:bold; }
.ack-title { font-weight:bold; text-decoration:underline; margin:16px 0 8px; font-size:12pt; }
.ack-para  { line-height:1.6; margin-bottom:22px; font-size:12pt; }
.ack-grid  { display:table; width:100%; font-size:12pt; }
.ack-row   { display:table-row; }
.ack-lbl   { display:table-cell; width:100px; padding:0 8px 20px 0; white-space:nowrap; }
.ack-line  { display:table-cell; border-bottom:1px solid #000; }
.ack-gap   { display:table-cell; width:40px; }
@media print {
  body { margin:0; }
  ...
  // ── BODY HTML (pure content, no header/footer) ───────────────────────────
...
<p class="salut">Dear ${ol.candidate_name.split(' ').pop()},</p>
...
<p class="sec">OTHER RULES AND REGULATION:</p>
...
<p class="sig-intro">Yours truly,<br>From <strong>Krishi Care &amp; Management Services Private Limited,</strong></p>
...
<table class="sig-table" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td class="sig-td">
      ...
      <p class="sig-lbl">( Authorized Signatory)</p>
      <p class="sig-sub">Human Resource</p>
    </td>
  </tr>
</table>
...
<div class="ann-meta">
  ...
  <div>Annual Cost to Company &ndash; Rs.${Number(ctcAnnual).toLocaleString('en-IN')} (Rupees ${numberToWords(Math.round(ctcAnnual))} Only)</div>
</div>
...
<table class="sal">
  ...
  <tbody>
    ...
    ${conv>0?`<tr><td class="c-sr">3</td><td>Conveyance Allowances</td><td class="c-num">${fmtV(conv)}</td><td class="c-num">${fmtV(conv*12)}</td></tr>`:''}
    <tr><td class="c-sr">${conv>0?4:3}</td><td>Other Allowances</td><td class="c-num">${fmtV(other)}</td><td class="c-num">${fmtV(other*12)}</td></tr>
    <tr><td class="c-sr">${conv>0?5:4}</td><td>Gratuity</td><td class="c-num">${fmtV(gratuity)}</td><td class="c-num">${fmtV(gratuity*12)}</td></tr>
    <tr class="r-bold"><td class="c-sr">${conv>0?6:5}</td><td>Gross Pay</td><td class="c-num">${fmtV(gross)}</td><td class="c-num">${fmtV(gross*12)}</td></tr>
    <tr><td class="c-sr">${conv>0?7:6}</td><td>Provident Fund</td><td class="c-num">${pfEmp>0?fmtV(pfEmp):'&ndash;'}</td><td class="c-num">${pfEmp>0?fmtV(pfEmp*12):'&ndash;'}</td></tr>
    <tr><td class="c-sr">${conv>0?8:7}</td><td>Professional Tax</td><td class="c-num">${pt>0?fmtV(pt):'&ndash;'}</td><td class="c-num">${pt>0?fmtV(pt*12):'&ndash;'}</td></tr>
    <tr class="r-bold"><td class="c-sr">${conv>0?9:8}</td><td>Total Deduction</td><td class="c-num">${fmtV(totalDed)}</td><td class="c-num">${fmtV(totalDed*12)}</td></tr>
    <tr class="r-bold"><td class="c-sr">${conv>0?10:9}</td><td>Net Salary (Gross - Total Deduction)</td><td class="c-num">${fmtV(netSalary)}</td><td class="c-num">${fmtV(netSalary*12)}</td></tr>
    <tr><td class="c-sr">${conv>0?11:10}</td><td>Employer PF contribution</td><td class="c-num">${pfEmpr>0?fmtV(pfEmpr):'&ndash;'}</td><td class="c-num">${pfEmpr>0?fmtV(pfEmpr*12):'&ndash;'}</td></tr>
    <tr><td class="c-sr">${conv>0?12:11}</td><td>Employer PF contribution Admin charges</td><td class="c-num">${pfAdmin>0?fmtV(pfAdmin):'&ndash;'}</td><td class="c-num">${pfAdmin>0?fmtV(pfAdmin*12):'&ndash;'}</td></tr>
    <tr class="r-bold"><td class="c-sr">${conv>0?13:12}</td><td>Total Compensation Package</td><td class="c-num">${fmtV(ctcMonthly)}</td><td class="c-num">${fmtV(ctcAnnual)}</td></tr>
  </tbody>
</table>
...
<div class="ack-grid">
  <div class="ack-row">
    <div class="ack-lbl">Signature:</div><div class="ack-line">&nbsp;</div>
    <div class="ack-gap"></div>
    <div class="ack-lbl">Date:</div><div class="ack-line">&nbsp;</div>
  </div>
  <div class="ack-row" style="height:8px"></div>
  <div class="ack-row">
    <div class="ack-lbl">Name:</div><div class="ack-line">&nbsp;</div>
    <div class="ack-gap"></div>
    <div class="ack-lbl">Location:</div><div class="ack-line">&nbsp;</div>
  </div>
</div>

</body></html>`;

  // Preview HTML (browser) — shows inline header+footer per page for screen
  const previewHtml = `<!DOCTYPE html><html><head>
<meta charset="UTF-8"><title>Offer Letter - ${ol.candidate_name}</title>
<style>
${css}
body { background:#888; padding:20px 0; }
.pg  { width:210mm; min-height:297mm; margin:0 auto 28px; background:#fff;
       box-shadow:0 2px 16px rgba(0,0,0,.3); display:flex; flex-direction:column; }
.pg-hdr { padding:8px 15mm 0; flex-shrink:0; }
.pg-hdr table { width:100%; border-collapse:collapse; }
.pg-hdr .logo-td { width:90px; vertical-align:middle; padding-right:10px; }
.pg-hdr .logo-img { width:80px; height:80px; object-fit:contain; display:block; }
.pg-hdr .tag { font-size:7pt; font-weight:bold; line-height:1.25; margin-top:3px; text-align:center; width:80px; }
.pg-hdr .company { font-size:18pt; font-weight:bold; line-height:1.15; }
.pg-hdr .addr { font-size:9pt; line-height:1.5; margin-top:3px; }
.pg-hdr .rule { border-top:1.5px solid #000; margin-top:6px; }
.pg-body { padding:14px 15mm 10px; flex:1; }
.pg-ftr { padding:4px 15mm 10px; margin-top:auto; flex-shrink:0; }
.pg-ftr .rule { border-top:1px solid #000; margin-bottom:4px; }
.pg-ftr .corp { text-align:center; font-size:9pt; }
.pg-ftr .cin  { text-align:center; font-size:9pt; font-weight:bold; }
</style></head><body>
<div class="pg">
  <div class="pg-hdr">
    <table cellpadding="0" cellspacing="0"><tr>
      <td class="logo-td"><img src="${LOGO}" class="logo-img" alt=""><div class="tag">BRINGING KRISHI &amp;<br>VIGYAN TOGETHER</div></td>
      <td><div class="company">Krishi Care &amp; Management Services Private Limited</div>
          <div class="addr"><strong>Regd. &amp; Head Office:</strong> 617, 6th Floor, Hubtown Viva, Western Express Highway, Shankarwadi, Jogeshwari (East), Mumbai - 400060.</div>
          <div class="addr">Email: recruitments@krishicare.in, Website: www.krishicare.in, Tel. - +912268284109</div>
      </td>
    </tr></table>
  ...
  <div class="pg-body">
    ...
    <p class="para">We are pleased to issue this letter of offer on the following terms &amp; conditions:</p>
    <p class="sec">EMOLUMENTS:</p>
    <p class="para">Your compensation on a cost to company basis will be <strong>Rs.${Number(ctcAnnual).toLocaleString('en-IN')} /- PA (Rupees ${numberToWords(Math.round(ctcAnnual))} Only)</strong>. The remuneration has taken into consideration the status and responsibility of the appointment.</p>
    <p class="sec">RESPONSIBILITIES:</p>
    <p class="para">You will work as <strong>&ldquo;${ol.designation}&rdquo;</strong> of the Company and will be responsible for carrying out the operations as directed by the management.</p>
    <p class="sec">PROBATION PERIOD:</p>
    <p class="para">You will be on a probationary period of <strong>${probStr} months</strong>.</p>
    <p class="sec">SEPERATION OF SERVICES:</p>
    <p class="para">Severance of relationship can be done by giving <strong>${noticeStr} month</strong> written notice.</p>
    <p class="sec">OTHER RULES AND REGULATION:</p>
    <p class="para">Please download PDF for full document.</p>
  </div>
  <div class="pg-ftr">
    <div class="rule"></div>
    <p class="corp"><strong>Corporate Office:</strong> H-12, Green Park Extension, New Delhi -110016. Tel: 011-41039506.</p>
  ...
  return { html: previewHtml, pdfHtml, headerHtml, footerHtml };
}
...
exports.preview = async (req, res) => {
  try {
    ...
    let { html } = buildOfferLetterHTML(ol);  // html = previewHtml for browser
...
exports.sendEmail = async (req, res) => {
  try {
    ...
    const { pdfHtml: offerHTML, headerHtml: offerHeaderHtml, footerHtml: offerFooterHtml, html: previewHTML } = buildOfferLetterHTML(ol);
