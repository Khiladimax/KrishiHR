...
// Called internally when payroll is processed for an employee assigned to a project
exports.hookPayrollExpenditure = async (employeeId, amount, month, year, payrollId) => {
  try {
    ...
    const projects = await db.query(
      `SELECT project_id FROM project_employees WHERE employee_id=$1 AND is_active=true`,
      [employeeId]
    );
    for (const row of projects.rows) {
      // Check not already recorded for this month/year
      ...
      if (!exists.rows.length) {
        await db.query(`
          INSERT INTO project_expenditures
            (project_id, employee_id, type, reference_id, amount, description, month, year, recorded_by)
          VALUES ($1,$2,'salary',$3,$4,$5,$6,$7,0)
        `, [row.project_id, employeeId, payrollId, parseFloat(amount),
            `Salary for ${month}/${year}`, month, year]);
      }
    }
  } catch (err) {
