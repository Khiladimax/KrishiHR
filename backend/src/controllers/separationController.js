  ...
  try {
    ...
    if (emp.reporting_manager_id) {
      emailEmployee(emp.reporting_manager_id,
        '🔔 Resignation Received — Action Required',
        `<p>Hi,</p><p><b>${emp.first_name} ${emp.last_name} (${emp.employee_code})</b> has submitted a resignation.</p><p>Last Working Day: <b>${lwd}</b></p><p>Please login to KrishiHR to review and approve.</p>`
      );
    }
    res.status(201).json({
      success: true,
...
exports.managerAction = async (req, res) => {
  ...
  try {
    ...
    emailRoleApprovers('hr',
      ...
      `<p>Hi,</p><p><b>${s.first_name} ${s.last_name} (${s.employee_code})</b> — resignation approved by Manager.</p><p>Last Working Day: <b>${finalLWD}</b></p><p>Please login to KrishiHR to review and approve (L2 HR step).</p>`
    );
...
exports.hrAction = async (req, res) => {
  ...
  try {
    ...
    emailRoleApprovers('accounts',
      ...
      `<p>Hi,</p><p><b>${s.first_name} ${s.last_name} (${s.employee_code})</b> — resignation approved by HR.</p><p>Last Working Day: <b>${approvedLWD}</b></p><p>Please login to KrishiHR to review and approve (L3 Accounts step).</p>`
    );
...
exports.accountsAction = async (req, res) => {
  ...
  try {
    ...
    emailRoleApprovers('admin',
      ...
      `<p>Hi,</p><p><b>${s.first_name} ${s.last_name} (${s.employee_code})</b> — resignation approved by Accounts.</p><p>Last Working Day: <b>${approvedLWD}</b></p><p>Please login to KrishiHR to give final approval (L4 Admin step).</p>`
    );
