...
exports.requestRegularization = async (req, res) => {
  ...
  try {
    ...
    if (req.user.employee_code === 'KC718') {
...
exports.getRegularizations = async (req, res) => {
  try {
    ...
    if (role === 'super_admin') {
      ...
      scopeCond = `AND e.employee_code = 'KC718'`;
    } else if (role === 'hr') {
...
exports.actionRegularization = async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'super_admin') {
      ...
      if (empCheck.rows[0]?.employee_code !== 'KC718') {
        return res.status(403).json({
          success: false,
...
exports.applyOD = async (req, res) => {
  ...
  try {
    ...
    const isKC718od = req.user.employee_code === 'KC718';
...
exports.applyWFH = async (req, res) => {
  ...
  try {
    ...
    const isKC718 = req.user.employee_code === 'KC718';
...
exports.markRange = async (req, res) => {
  ...
  try {
    ...
    if (role !== 'super_admin' && empCode !== 'KC718')
      return res.status(403).json({ success: false, message: 'Not authorized' });
  ...
  try {
    ...
    const pts = await db.query(
      `SELECT m.employee_id,
              CONCAT(e.first_name,' ',e.last_name)  AS emp_name,
              e.employee_code,
              TO_CHAR(m.logged_at AT TIME ZONE 'Asia/Kolkata', 'YYYY-MM-DD') AS date,
              m.lat::float AS lat,
              m.lng::float AS lng,
              m.logged_at
      ...
      `SELECT DISTINCT e.id AS employee_id,
       ...
       WHERE NOT EXISTS (
         ...
         WHERE m2.employee_id = candidates.id
           AND TO_CHAR(m2.logged_at AT TIME ZONE 'Asia/Kolkata','YYYY-MM-DD') = candidates.day_date
       )
