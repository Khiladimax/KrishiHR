...
exports.getLeaderboard = async (req, res) => {
  try {
    ...
    let dateFilter = '';
    if (period === 'week') {
      dateFilter = `AND gr.answered_at AT TIME ZONE 'Asia/Kolkata' >= DATE_TRUNC('week', NOW() AT TIME ZONE 'Asia/Kolkata')`;
    } else if (period === 'month') {
      dateFilter = `AND gr.answered_at AT TIME ZONE 'Asia/Kolkata' >= DATE_TRUNC('month', NOW() AT TIME ZONE 'Asia/Kolkata')`;
    } else if (period === 'all') {
      dateFilter = '';
    } else {
...
exports.exportScores = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition',`attachment; filename=KrishiHR_GK_Scores_${year}.xlsx`);
    res.send(buf);
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: 'Server error' });
...
exports.exportYearly = async (req, res) => {
  try {
    ...
    const wb = XLSX.utils.book_new();
    const wsData = [
      [`🏆 KrishiHR GK CHAMPION LEADERBOARD ${year}`],
    ...
    const buf = XLSX.write(wb, { type:'buffer', bookType:'xlsx' });
    res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition',`attachment; filename=KrishiHR_GK_Champion_${year}.xlsx`);
    res.send(buf);
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: 'Server error' });
...
exports.exportResponses = async (req, res) => {
  try {
    ...
    const buf = XLSX.write(wb, { type:'buffer', bookType:'xlsx' });
    res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition',`attachment; filename=KrishiHR_GK_Responses_${year}.xlsx`);
    res.send(buf);
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: 'Server error' });
